...
local get_attached_null_ls_sources = function()
  ...
  for _, source in pairs(null_ls_sources) do
    if source.filetypes[filetype] then
      if not vim.tbl_contains(ret, source.name) then table.insert(ret, source.name) end
    end
  end
  return ret
end
...
local config = {
  ...
  tabline = {},
  extensions = {
    'quickfix', 'nvim-dap-ui', 'nvim-tree', 'toggleterm', get_extension_center('Alpha α', 'alpha'),
